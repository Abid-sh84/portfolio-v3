...
export function getBlogBySlug(slug) {
  ...
  return {
    ...
    frontmatter: {
      ...
      lastModified: frontmatter.lastModified || null, // used for dateModified in JSON-LD + "Last updated" display
...
export function extractTOC(content) {
  ...
  const toc = [];
  const seenIds = {}; // track duplicate slugs
  let match;
  ...
  while ((match = headingRegex.exec(content)) !== null) {
    ...
    // Deduplicate: rehype-slug appends -1, -2, … for repeated slugs
    let id = baseId;
